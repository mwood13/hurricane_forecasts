...
# Load in storms
storms <- read_sf('Centers\\points_08.shp')

# Set up dates
dates<-seq(as.Date("2008-01-01"),as.Date("2008-12-31"),by = 1)

#week_end <- dates[weekdays(dates) %in%  c('Saturday')]

# create dataframe to hold variables
test_df <- data.frame(matrix(ncol=10,nrow=length(dates)*length(us_counties$NAME), 
...
write.csv(test_df, 'Daily Data/Daily2008.csv')
